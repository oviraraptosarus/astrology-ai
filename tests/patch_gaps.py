...
def clean_ocr_text(text: str) -> str:
    """
    Cleans raw OCR artifacts from scanned Sanskrit/Astrology PDF corpora:
    1. Fixes broken hyphenated words across line wraps (e.g. 'aspec-\nting' -> 'aspecting')
    2. Strips random OCR noise symbols and isolated orphaned consonants
    3. Normalizes whitespace and standardizes Sanskrit diacritic representations
    4. Removes non-printable unmapped glyphs while preserving English & Devanagari
    """
    if not text:
        return ""
    text = re.sub(r'(\w+)-\s*\n\s*(\w+)', r'\1\2', text)
