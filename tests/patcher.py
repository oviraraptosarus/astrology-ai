...
res = patch("E:/ASTROLOGY AI/rectification_engine.py", old_string=old_events, new_string=new_events)

old_csl = """    def _csl_significatorship_score(kp_chart: Dict, event_house: int) -> float:
        """
...
new_csl = """    def _csl_significatorship_score(kp_chart: Dict, primary_house: int, supporting_houses: List[int]) -> float:
        try:
            cusps = kp_chart["cusps"]
            abcd = kp_chart["abcd_significators"]
            cusp_sl = cusps[primary_house - 1]["sub_lord"]

            best_score = 0.0
            for h in supporting_houses:
                sig = abcd.get(str(h), {})
                level_a = sig.get("level_a", [])
                level_b = sig.get("level_b", [])
                level_c = sig.get("level_c", [])
                ...
                if cusp_sl in level_a: best_score = max(best_score, 1.0)
                elif cusp_sl in level_b: best_score = max(best_score, 0.85)
                elif cusp_sl in level_c: best_score = max(best_score, 0.7)
                elif cusp_sl in level_d: best_score = max(best_score, 0.6)
            
            return best_score
        except Exception:
            return 0.0"""
res2 = patch("E:/ASTROLOGY AI/rectification_engine.py", old_string=old_csl, new_string=new_csl)

old_dasha = """    def _dasha_connectivity_score(kp_chart: Dict, dasha: Dict[str, str], event_house: int) -> float:
...
new_dasha = """    def _dasha_connectivity_score(kp_chart: Dict, dasha: Dict[str, str], supporting_houses: List[int]) -> float:
        abcd = kp_chart.get("abcd_significators", {})
        all_sig = set()
        for h in supporting_houses:
            sig = abcd.get(str(h), {})
        ...
        overlap = dasha_lords & all_sig
        return len(overlap) / 3.0"""
res3 = patch("E:/ASTROLOGY AI/rectification_engine.py", old_string=old_dasha, new_string=new_dasha)
...
new_loop = """                total = 0.0
                ...
                for ev_type, ev_date_str, ev_utc in event_dates:
                    ...
                    dasha = RectificationEngine._dasha_on_date(birth_nak_lord, candidate_utc, ev_utc)
                    dasha_sc = RectificationEngine._dasha_connectivity_score(kp, dasha, supporting)
                    lagna_sc = RectificationEngine._lagna_nakshatra_score(lagna_lon, primary, kp)"""
res4 = patch("E:/ASTROLOGY AI/rectification_engine.py", old_string=old_loop, new_string=new_loop)

old_score = """                avg = total / len(event_dates)
                curve.append({"shift_minutes": step, "score": round(avg, 4)})"""

new_score = """                avg = total / len(event_dates)
                # Apply a slight center-gravity penalty (0.1% deduction per minute of deviation)
                # This ensures that if the math is tied between 7:15 and 8:59, standard hospital time breaks the tie.
                time_penalty = abs(step) * 0.001
                adjusted_score = max(0.0, avg - time_penalty)
                
                curve.append({"shift_minutes": step, "score": round(adjusted_score, 4)})"""
res5 = patch("E:/ASTROLOGY AI/rectification_engine.py", old_string=old_score, new_string=new_score)

print(f"Patches done. {res} {res2} {res3} {res4} {res5}")
